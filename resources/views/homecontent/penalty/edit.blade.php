        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    <div class="py-3 px-2">
                        <h3 class="font-weight-bold text-primary">Edit Penalty</h3>
                    </div>
                        ...
                        <div class="card-body">
                            <form action="{{ route('penalties.update', $penalty->id) }}" method="POST">
                                    ...
                                    <select name="user_id" id="user_id" class="form-control">
                                        @foreach ($users as $user)
                                            <option value="{{ $user->id }}"
                                                {{ $penalty->user_id == $user->id ? 'selected' : '' }}>
                                    ...
                                    <select name="enquire_id" id="enquire_id" class="form-control">
                                        @foreach ($enquiries as $enquiry)
                                            <option value="{{ $enquiry->id }}"
                                                {{ $penalty->enquire_id == $enquiry->id ? 'selected' : '' }}>
                                                {{ $enquiry->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <select name="booking_id" id="booking_id" class="form-control">
                                        @foreach ($bookings as $booking)
                                            <option value="{{ $booking->id }}"
                                                {{ $penalty->booking_id == $booking->id ? 'selected' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="reason" id="reason" class="form-control">{{ $penalty->reason }}</textarea>
                                </div>
